-- ================================================================
-- SUPABASE SECURITY FIXES - OPTIMIZED Row Level Security (RLS)
-- ================================================================
-- Run this script in Supabase SQL Editor to fix all security issues
-- This will enable RLS and create OPTIMIZED policies for all tables
-- 
-- PERFORMANCE OPTIMIZATION:
-- All auth.jwt() calls are wrapped in (SELECT ...) to prevent
...
-- ABSENTEES TABLE
DROP POLICY IF EXISTS "Staff can view their own absentees" ON public.absentees;
DROP POLICY IF EXISTS "Approved staff can insert absentees" ON public.absentees;
DROP POLICY IF EXISTS "Admins can update absentees" ON public.absentees;
DROP POLICY IF EXISTS "Admins can delete absentees" ON public.absentees;
DROP POLICY IF EXISTS "Service role has full access to absentees" ON public.absentees;

-- STUDENTS TABLE
DROP POLICY IF EXISTS "Approved users can view students" ON public.students;
DROP POLICY IF EXISTS "Admins can modify students" ON public.students;
DROP POLICY IF EXISTS "Service role has full access to students" ON public.students;

-- SEMESTERS TABLE
DROP POLICY IF EXISTS "Approved users can view semesters" ON public.semesters;
DROP POLICY IF EXISTS "Admins can modify semesters" ON public.semesters;
DROP POLICY IF EXISTS "Service role has full access to semesters" ON public.semesters;
...
DROP POLICY IF EXISTS "Users can update their own reset requests" ON public.password_reset_requests;
DROP POLICY IF EXISTS "Admins can view all reset requests" ON public.password_reset_requests;
DROP POLICY IF EXISTS "Service role has full access to reset requests" ON public.password_reset_requests;
...
DROP POLICY IF EXISTS "Admins can modify pending registrations" ON public.pending_registrations;
DROP POLICY IF EXISTS "Service role has full access to pending registrations" ON public.pending_registrations;


-- 3. CREATE SECURE OPTIMIZED RLS POLICIES
-- ================================================================
...
ON public.users
FOR SELECT
USING (auth.uid()::text = id::text OR email = (SELECT auth.jwt()->>'email'));
...
FOR SELECT
USING (
  marked_by = (SELECT auth.jwt()->>'email')
  OR EXISTS (
    SELECT 1 FROM public.users
    WHERE email = (SELECT auth.jwt()->>'email')
    AND role = 'admin'
...
-- 7. PERFORMANCE: All auth.jwt() calls wrapped in (SELECT ...)
--    to prevent re-evaluation for each row (10-100x faster queries)
-- ================================================================
